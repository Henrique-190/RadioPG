...
def diario(soup, day):
    ...
    for programa in ps.findAll('li', class_='all-100 column-group no-margin half-top-padding push-bottom quarter-vertical-padding'):
        for p in programa.findAll('tr'):
            if p.find('td', class_='txtGrey fw-500 quarter-right-padding') is not None:
                inicio = datetime.strptime(programa.find('td', class_='txtGrey fw-500 quarter-right-padding').text, '%Hh%M').time().strftime('%H:%M')
            else:
                hora = re.match(r'(\d{2}h\d{2})', p.find('a', class_='lnkBlack').text)
                if hora:
                    inicio = datetime.strptime(hora.group(1), '%Hh%M').time().strftime('%H:%M')
                else:
                    inicio = (datetime.strptime(programacao[-1].start,'%H:%M')  + timedelta(minutes=30)).time().strftime('%H:%M')
            link = None
...
def renascenca(headers):
    ...
    for p in ps.findAll('li', class_='all-100 column-group no-margin half-top-padding push-bottom quarter-vertical-padding'):
    ...
    return radio.Radio('Rádio Renascença', 'https://rrsite-images.azureedge.net/img/Renascenca-85-anos_logo.svg', 'https://rr.sapo.pt', programacao)
