...
def uteis(tabelas):
    ...
    for horas in tabelas.find_all('table', class_='day-schedule-hours'):
        ...
        if cadaHora:
            ...
            programacao[titulo + inicio + str([]) + str([translate[i] for i in range(5)])] = program.Program(titulo, "", inicio, "", [], [translate[i] for i in range(5)], "")

        
            for programas in horas.find_all('tr', class_='day-schedule-hours-program'):
                inicio = datetime.strptime(programas.find('td',class_='day-schedule-hours-program-hour').text.strip(), '%Hh%M').strftime('%H:%M')
                ...
                if programa:
                    ...
                    if lista:
                        for p in lista.find_all('li'):
                            ...
                            repetition = p.find('span',class_="repetition").text.strip() if p.find('span',class_="repetition") else ""
...
def observador(headers):
    ...
    response = requests.get(link, headers={"user-agent": 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.132 Safari/537.36'})
